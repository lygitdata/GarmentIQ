...
async function addDescriptionListItem(id, description) {
    ...
    funcSelector.id = `function-selector-${id}`;

    const defaultOption = document.createElement('option');
    defaultOption.textContent = 'Select function';
    defaultOption.disabled = true;
    defaultOption.selected = true;
    funcSelector.appendChild(defaultOption);

    for (const funcName in derivationDict) {
        const option = document.createElement('option');
        option.value = funcName;
        option.textContent = funcName;
        funcSelector.appendChild(option);
    }

    const inputContainer = document.createElement('div'); // holds dynamic inputs

    funcSelector.addEventListener('change', () => {
        // Clear previous input content
        inputContainer.innerHTML = '';

        const selectedFunc = funcSelector.value;
        const args = derivationDict[selectedFunc];
    ...
    // Add the default description input
    const label = document.createElement('label');
    ...
    li.append(label, input, document.createElement('br'), funcSelector, inputContainer);
    ul.appendChild(li);
}
...
function resetDescriptionListItems() {
    const svgId = garmentSelector.value;
    const svg = document.getElementById(svgId);
    if (!svg) {
        console.error(`SVG "#${svgId}" not found — cannot reset list`);
        return;
    }

    // 1) clear out all existing <li>
    const ul = document.getElementById('garmentCustomLandmarkNameList');
    ul.innerHTML = '';

    // 2) find all remaining custom points, in numeric order
    const customGs = Array.from(
        svg.querySelectorAll('g.landmark[data-custom="true"]')
    ).sort((a, b) => Number(a.dataset.id) - Number(b.dataset.id));

    // 3) re-create each list item
    customGs.forEach(g => {
        const id = Number(g.dataset.id);
        const description = g.dataset.description || '';
        addDescriptionListItem(id, description);
    });
}
...
async function saveSelection() {
    let derivationDict;
    try {
        derivationDict = await fetchDerivationDictionary();
    } catch (err) {
        alert("Error loading derivation definitions: " + err.message);
        return;
    }
    if (!derivationDict) {
        alert("No derivation definitions received.");
        return;
    }
    ...
    selectedPoints.forEach((idSet, garmentName) => {
        ...
        if (!svg) {
            alert(`SVG "#${garmentName}" not found — skipping`);
            return;
        }
        ...
        sortedIds.forEach(id => {
            ...
            if (isCustom) {
                const li = document.querySelector(`#garmentCustomLandmarkNameList li[data-id="${id}"]`);
                if (!li) {
                    alert(`Could not find form elements for custom point ${id}`);
                    return;
                }

                // Function name
                const fnSelect = li.querySelector(`#function-selector-${id}`);
                const fnName = fnSelect?.value;
                if (!fnName || !derivationDict[fnName]) {
                    alert(`Please select a valid function for point ${id}`);
                    return;
                }
                ...
                const fnArgs = derivationDict[fnName];

                for (const key in fnArgs) {
                    const input = li.querySelector(`[name="${key}"]`);
                    if (!input || !input.value) {
                        alert(`Please fill in "${key}" for point ${id}`);
                        return;
                    }
                    derivation[key] = input.value;
                }
