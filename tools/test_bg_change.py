...
def main():
    ...
    for idx, entry in enumerate(valid_dataset.db):
        ...
        # 6.2.5) build a background‐swapped version for pose
        # orig: H×W×3 float in [0,1], mask: H×W (0/1)
        bg_img = giq.segmentation.change_background_color(
            image_np     = (orig * 255).astype(np.uint8),
            mask_np      = (mask.squeeze(-1) if mask.ndim==3 else mask)*255,
            background_color = [102,255,102]
        )
        # save to a temp file
        tmp_bg = os.path.join(final_output_dir, f"__bg_{idx}_{os.path.basename(img_path)}")
        cv2.imwrite(tmp_bg, cv2.cvtColor(bg_img, cv2.COLOR_RGB2BGR))

        # 6.3) landmark detect (using swapped‐BG image)
        # monkey‐patch the dataset entry so it reads our bg image
        entry = valid_dataset.db[idx]
        orig_path = entry['image']
        entry['image'] = tmp_bg

        # now this will load, crop & warp the bg‐swapped image under the hood
        hr_input, _, _, meta = valid_dataset[idx]

        # restore the original path so nothing else breaks
        entry['image'] = orig_path
